...
class TileLayoutTest {
    ...
    private fun sampleData() =
        CachedTripsData(
            ...
            trips =
                listOf(
                    TripDto("2026-11-02T11:07:00Z", delayMinutes = 0, track = "4b", durationMinutes = 33, transfers = 0, cancelled = false),
                    TripDto("2026-11-02T11:18:00Z", delayMinutes = 5, track = "2", durationMinutes = 28, transfers = 0, cancelled = false),
                    TripDto("2026-11-02T11:33:00Z", delayMinutes = 0, track = "3", durationMinutes = 39, transfers = 1, cancelled = true),
                ),
    ...
    @Test
    fun `header shows the direction label`() {
    ...
    fun `delayed row shows a plus-N marker`() {
        val layout = layoutFor(Direction.AB, CacheState.Fresh(sampleData()))
        LayoutElementAssertionsProvider(layout).onElement(hasText("+5")).assertExists()
    }

    @Test
    fun `cancelled row is marked cancelled`() {
        val layout = layoutFor(Direction.AB, CacheState.Fresh(sampleData()))
        LayoutElementAssertionsProvider(layout).onElement(hasText("Cancelled")).assertExists()
    }

    @Test
    fun `stale state shows an age indicator in the header`() {
        val layout = layoutFor(Direction.AB, CacheState.Stale(sampleData()))
        LayoutElementAssertionsProvider(layout).onElement(containsText("min old")).assertExists()
    }
